...
def scatter_ICs(T0, grid, phys, comm, rank, size, ICtype = "broad_random"):
    ...
    X = gridS.X; Xn= [gridS.X[0]/grid.L[0],gridS.X[1]/grid.L[1]]
    ...
    if rank == 0:
        ...
        if ICtype == "broad_random":
            ...
            q0  = phys.psi_amp*np.cos(2*np.pi*Xn[0])*np.cos(2*np.pi*Xn[1])
            ...
            q0  = phys.psi_amp*np.cos(4*np.pi*Xn[0])*np.cos(4*np.pi*Xn[1])
            ...
            q0  = phys.psi_amp*np.sin(2*np.pi*Xn[0])*np.sin(2*np.pi*Xn[1])
            A0  = 0*q0

        elif ICtype == "row_vortex":

            q0  = phys.psi_amp*np.sin(4*np.pi*Xn[0])*np.cos(4*np.pi*Xn[1])
    ...
    return q_split[rank], A_split[rank]
