...
fig_HEadv, axesHEadv = plt.subplots(nrows=1, ncols=1, sharex=True, sharey=True, figsize=figsize)
fig_HElor, axesHElor = plt.subplots(nrows=1, ncols=1, sharex=True, sharey=True, figsize=figsize)
fig_HEvis, axesHEvis = plt.subplots(nrows=1, ncols=1, sharex=True, sharey=True, figsize=figsize)
fig_MEadv, axesMEadv = plt.subplots(nrows=1, ncols=1, sharex=True, sharey=True, figsize=figsize)
fig_MEvis, axesMEvis = plt.subplots(nrows=1, ncols=1, sharex=True, sharey=True, figsize=figsize)
fig_TEdyn, axesTEdyn = plt.subplots(nrows=1, ncols=1, sharex=True, sharey=True, figsize=figsize)
fig_TEvis, axesTEvis = plt.subplots(nrows=1, ncols=1, sharex=True, sharey=True, figsize=figsize)
...
for index in range(len(mag_values)):
    ...
    axesHEadv.plot(kbin, HEadv, lw=lw, linestyle = lines_index[index], color=color_index[index])
    axesHElor.plot(kbin, HElor, lw=lw, linestyle = lines_index[index], color=color_index[index])
    axesHEvis.plot(kbin, HEvis, lw=lw, linestyle = lines_index[index], color=color_index[index])
    axesMEadv.plot(kbin, MEadv, lw=lw, linestyle = lines_index[index], color=color_index[index])
    axesMEvis.plot(kbin, MEvis, lw=lw, linestyle = lines_index[index], color=color_index[index])
    axesTEdyn.plot(kbin, TEdyn, lw=lw, linestyle = lines_index[index], color=color_index[index])
    axesTEvis.plot(kbin, TEvis, lw=lw, linestyle = lines_index[index], color=color_index[index])

axesHEadv.set_ylabel(r'$T_{q}$')
axesHElor.set_ylabel(r'$T_{L}$')
axesMEadv.set_ylabel(r'$T_{A}$')
axesHEvis.set_ylabel(r'$-2R_e^{-1}k^2\hat E_H$')
axesMEvis.set_ylabel(r'$-2R_m^{-1}k^2\hat E_M$')
axesTEdyn.set_ylabel(r'$T_q + T_L + T_A$')
axesTEvis.set_ylabel(r'$-2R_e^{-1}k^2\hat E_H - 2R_m^{-1}k^2\hat E_M$')

for axes in [axesHEadv,axesHElor,axesHEvis,axesMEadv,axesMEvis,axesTEdyn,axesTEvis]:
    axes.set_xscale('log')
...
for axes in [axesHEadv,axesHElor,axesMEadv,axesTEdyn]:
    low, high = axes.get_ylim()
    bound = max(abs(low), abs(high))
    axes.set_ylim([-bound, bound])

for fig in [fig_HEadv,fig_HElor,fig_HEvis,fig_MEadv,fig_MEvis,fig_TEdyn,fig_TEvis]:
    fig.tight_layout()

if save_format == 'png' or save_format == 'both':
    fig_HEadv.savefig("qgmhd_HEadv_trans_N{}.png".format(N[0]))
    fig_HElor.savefig("qgmhd_HElor_trans_N{}.png".format(N[0]))
    fig_HEvis.savefig("qgmhd_HEvis_trans_N{}.png".format(N[0]))
    fig_MEadv.savefig("qgmhd_MEadv_trans_N{}.png".format(N[0]))
    fig_MEvis.savefig("qgmhd_MEvis_trans_N{}.png".format(N[0]))
    fig_TEdyn.savefig("qgmhd_TEdyn_trans_N{}.png".format(N[0]))
    fig_TEvis.savefig("qgmhd_TEvis_trans_N{}.png".format(N[0]))
if save_format == 'eps' or save_format == 'both':
    fig_HEadv.savefig("qgmhd_HEadv_trans_N{}.eps".format(N[0]))
    fig_HElor.savefig("qgmhd_HElor_trans_N{}.eps".format(N[0]))
    fig_HEvis.savefig("qgmhd_HEvis_trans_N{}.eps".format(N[0]))
    fig_MEadv.savefig("qgmhd_MEadv_trans_N{}.eps".format(N[0]))
    fig_MEvis.savefig("qgmhd_MEvis_trans_N{}.eps".format(N[0]))
    fig_TEdyn.savefig("qgmhd_TEdyn_trans_N{}.eps".format(N[0]))
    fig_TEvis.savefig("qgmhd_TEvis_trans_N{}.eps".format(N[0]))
...
for index in range(len(mag_values)):
    ...
    if normalize: 
        ...
        if maxTEdyn > 0: TEdyn = TEdyn/maxTEdyn;
        if maxTEvis > 0: TEvis = TEvis/maxTEvis;
    print("For M2 = ",M2_vector[index],": (flux)")
    print("Max HEvis: ",maxHEvis)
    print("Max MEvis: ",maxMEvis)
    print("Max HEadv: ",maxHEadv)
    print("Max HElor: ",maxHElor)
...
axesHEadv.set_ylabel(r'$-\int T_{q}(k) dk$')
axesHElor.set_ylabel(r'$-\int T_{L}(k) dk$')
axesMEadv.set_ylabel(r'$-\int T_{A}(k) dk$')
axesHEvis.set_ylabel(r'$2R_e^{-1}\int k^2\hat E_Hdk$')
axesMEvis.set_ylabel(r'$2R_m^{-1}\int k^2\hat E_Mdk$')
axesTEdyn.set_ylabel(r'$-\int T_q + T_L + T_A dk$')
axesTEvis.set_ylabel(r'$2\int R_e^{-1}k^2\hat E_H + R_m^{-1}k^2\hat E_M dk$')
...
if save_format == 'png' or save_format == 'both':
    fig_HEadv.savefig("qgmhd_HEadv_flux_N{}.png".format(N[0]))
    fig_HElor.savefig("qgmhd_HElor_flux_N{}.png".format(N[0]))
    fig_HEvis.savefig("qgmhd_HEvis_flux_N{}.png".format(N[0]))
    fig_MEadv.savefig("qgmhd_MEadv_flux_N{}.png".format(N[0]))
    fig_MEvis.savefig("qgmhd_MEvis_flux_N{}.png".format(N[0]))
    fig_TEdyn.savefig("qgmhd_TEdyn_flux_N{}.png".format(N[0]))
    fig_TEvis.savefig("qgmhd_TEvis_flux_N{}.png".format(N[0]))
if save_format == 'eps' or save_format == 'both':
    fig_HEadv.savefig("qgmhd_HEadv_flux_N{}.eps".format(N[0]))
    fig_HElor.savefig("qgmhd_HElor_flux_N{}.eps".format(N[0]))
    fig_HEvis.savefig("qgmhd_HEvis_flux_N{}.eps".format(N[0]))
    fig_MEadv.savefig("qgmhd_MEadv_flux_N{}.eps".format(N[0]))
    fig_MEvis.savefig("qgmhd_MEvis_flux_N{}.eps".format(N[0]))
    fig_TEdyn.savefig("qgmhd_TEdyn_flux_N{}.eps".format(N[0]))
    fig_TEvis.savefig("qgmhd_TEvis_flux_N{}.eps".format(N[0]))
