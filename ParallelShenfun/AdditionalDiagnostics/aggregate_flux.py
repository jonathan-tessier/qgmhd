...
# post-processing script to compute the spectral energy flux from 
...
#
# Current outputs: -\int T(k) components, viscous and full equations
#
...
for index in range(len(mag_values)):
    ...
    p1, = axesDyn[axii].plot(kbin, HEadv, '-r', label=r'$-\int T_{q}(k) dk$')
    p3, = axesDyn[axii].plot(kbin, HElor, '--r', label=r'$-\int T_{L}(k) dk$')

    twin1 = axesDyn[axii].twinx()
    p4, = twin1.plot(kbin, MEadv, '--b', label=r'$-\int T_{A}(k) dk$')

    twin2 = axesVis[axii].twinx() 
    p2, = axesVis[axii].plot(kbin, HEvis, '-r', label=r'$2R_e^{-1}\int k^2\hat E_Hdk$')
    p5, = twin2.plot(kbin, MEvis, '-b', label=r'$2R_m^{-1}\int k^2\hat E_Mdk$')

    twin3 = axesTot[axii].twinx() 
    p6, = axesTot[axii].plot(kbin, HEtot, '-r', label=r'$\int 2R_e^{-1}k^2\hat E_H - T_q - T_L dk$')
    p7, = twin3.plot(kbin, MEtot, '-b', label=r'$\int 2R_m^{-1}k^2\hat E_M - T_A dk$')
    ...
    axesVis[axii].set_ylim(0,np.max(abs(HEvis)))
    twin2.set_ylim(0,np.max(abs(MEvis)))
...
fig_Dyn.savefig("qgmhd_DynFlux_N{}.png".format(N[0]))
fig_Vis.tight_layout()
fig_Vis.subplots_adjust(wspace=0.5, bottom=0.25)
fig_Vis.savefig("qgmhd_VisFlux_N{}.png".format(N[0]))
fig_Tot.tight_layout()
fig_Tot.subplots_adjust(wspace=0.5, bottom=0.25)
fig_Tot.savefig("qgmhd_TotFlux_N{}.png".format(N[0]))
