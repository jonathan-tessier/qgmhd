...
# Compute a radial coordinates (p for plotting)
...
for ii in time_vector:
    ...
    for jj in range(1, len(kbin)-1): # ignore largest scale integral diverges..
    ...
    KE_advect_bin  =  -np.cumsum(KE_advect_bin)
    KE_lorentz_bin  =  -np.cumsum(KE_lorentz_bin)
    KE_viscous_bin  =  -np.cumsum(KE_viscous_bin)
    ...
    plt.subplot(1,2,1)
    plt.xscale('log')
    plt.plot(kbin[1:], KE_advect_bin[1:], '-g', label=r'$-\int T_{q}(k) dk$')
    plt.plot(kbin[1:], KE_lorentz_bin[1:], '-b', label=r'$-\int T_{L}(k) dk$')
    plt.plot(kbin[1:], ME_advect_bin[1:], '-r', label=r'$-\int T_{A}(k) dk$')
    plt.plot(kbin[1:],KE_advect_bin[1:]+KE_lorentz_bin[1:]+ ME_advect_bin[1:], '-k', label=r'$\Pi(k)$')
    ...
    plt.title('Dynamic Fluxes at t = %4.2f' % t)

    plt.subplot(1,2,2)
    plt.xscale('log')
    plt.plot(kbin[1:], KE_viscous_bin[1:], '-r', label=r'$2\mu \int k^2\hat E_V(k) dk$')
    plt.plot(kbin[1:], ME_viscous_bin[1:], '-b', label=r'$2\eta \int k^2\hat E_M(k) dk$')
    plt.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
    plt.grid(True)
    plt.legend(loc='upper left');
    plt.xlabel("wavenumber (k)")
    plt.title('Viscous Fluxes at t = %4.2f' % t)
...
plt.subplot(1,2,1)
#plt.xscale('log')
plt.plot(kbin[1:], KEadv[1:], '-g', label=r'$-\int T_{q}(k) dk$')
plt.plot(kbin[1:], KElor[1:], '-b', label=r'$-\int T_{L}(k) dk$')
plt.plot(kbin[1:], MEadv[1:], '-r', label=r'$-\int T_{A}(k) dk$')
plt.plot(kbin[1:], Total[1:], '-k', label=r'$\Pi(k)$')
#plt.xlim([0,180])
plt.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
plt.grid(True)
plt.legend(loc='best');
plt.xlabel("wavenumber (k)")
plt.title('Time-averaged Dynamic Fluxes')

plt.subplot(1,2,2)
#plt.xscale('log')
plt.plot(kbin[1:], KEvis[1:], '-r', label=r'$2\mu \int k^2\hat E_V(k) dk$')
plt.plot(kbin[1:], MEvis[1:], '-b', label=r'$2\eta \int k^2\hat E_M(k) dk$')
plt.grid(True)
plt.legend(loc='best');
plt.xlabel("wavenumber (k)")
#plt.xlim([0,180])
plt.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
plt.title('Time-averaged Viscous Fluxes')
plt.tight_layout()
plt.show()
